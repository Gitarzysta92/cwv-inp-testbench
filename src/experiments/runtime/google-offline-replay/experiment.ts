#!/usr/bin/env node
/**
 * Google runtime CDP smoke test — capture + offline replay.
 *
...
import { GOOGLE_APP_URL, GOOGLE_BLOCK_SCRIPT_PATTERNS } from '../../lab/google-web-vitals-probe/profiles';
import { runGoogleOfflineReplaySmoke } from './run';

export type StackMode = 'docker' | 'local';

export async function runGoogleOfflineReplayExperiment(stackMode?: StackMode): Promise<void> {
  ...
  try {
    const results = await runGoogleOfflineReplaySmoke(client);
...
runGoogleOfflineReplayExperiment(stackMode).catch((err) => {
  console.error(err);
  process.exit(1);
});
