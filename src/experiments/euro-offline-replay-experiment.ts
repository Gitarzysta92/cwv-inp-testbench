#!/usr/bin/env node
/**
 * Euro.com.pl runtime experiment.
 *
 *   npx tsx src/experiments/euro-offline-replay-experiment.ts --local
 *   npx tsx src/experiments/euro-offline-replay-experiment.ts --docker
 *
 * Flow:
 * 1. Prepare a live Euro profile with third-party script block patterns.
...
import {
  ...
  euroLiveProfile,
} from './euro-offline-replay-fixtures';
...
async function runEuroExperiment(client: RuntimeTestClient): Promise<void> {
  const profile = euroLiveProfile();
  ...
  try {
    ...
    const networkStats: EuroNetworkStats = {
      capture: {
...
async function main(): Promise<void> {
  ...
  try {
    await runEuroExperiment(client);
  } finally {
    if (stack) {
      console.error('\nStopping stack…');
      await stack.stop();
